...
class StaffWindow(QMainWindow):
    ...
    def update_cart_display(self):
        ...
        if self.selected_customer:
            if self.selected_customer[5] == 'vip':
                discount = subtotal * Decimal('0.15')  # 15% for VIP
            elif self.selected_customer[5] == 'student':
    ...
    def select_customer(self):
        ...
        if dialog.exec():
            ...
            if self.selected_customer:
                self.customer_label.setText(f"👤 {self.selected_customer[1]} ({self.selected_customer[5].upper()})")
                self.update_cart_display()
            else:
...
class CustomerSelectionDialog(QDialog):
    ...
    def display_customers(self, customers):
        ...
        for row, customer in enumerate(customers):
            ...
            self.customers_table.setItem(row, 3, QTableWidgetItem(customer[5]))
    
    def filter_customers(self):
        search_text = self.search_input.text().lower()
        filtered = [c for c in self.all_customers 
                   if search_text in c[1].lower() or (c[2] and search_text in c[2].lower())]
